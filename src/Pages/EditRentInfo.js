...
const EditRentInfo = () => {
  ...
  useEffect(() => {
    const fetchRentData = async () => {
      ...
      const rentSnapshot = await getDoc(rentDoc);
      if (rentSnapshot.exists()) {
        setFormData(rentSnapshot.data());
      } else {
  ...
  const handleDeleteFile = (field, index) => {
    if (field === 'imageUrls' && index !== undefined) {
      setFormData(prevState => ({
        ...prevState,
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (formData.newVideo && formData.newVideo instanceof File) {
        const videoRef = ref(imageDB, `forrent/${uuidv4()}`);
      ...
      if (formData.new360Image && formData.new360Image instanceof File) {
        const image360Ref = ref(imageDB, `forrent/${uuidv4()}`);
      ...
      if (formData.newFloorPlan && formData.newFloorPlan instanceof File) {
        const floorPlanRef = ref(imageDB, `forrent/${uuidv4()}`);
      ...
      await updateDoc(rentDoc, updatedData);
  ...
  const handleAlertClose = () => {
    setShowAlert(false);
    if (isSuccess) {
      // Navigate to the refreshed RentListingInfo page for the specific listing
      navigate(`/Rent`);
    }
  };
